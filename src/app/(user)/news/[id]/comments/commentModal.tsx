...
export function CommentModal({
	...
	const [formData, setFormData] = useState({
		...
		code: comment.code,
		prog_language: comment.prog_language,
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		const { content, code, prog_language, files, attachments, deleteFiles, deleteAttachments } =
			formData;
	...
	return (
		<div className='w-full h-full flex justify-center items-center bg-black bg-opacity-60 fixed top-0 left-0 z-[11]'>
			...
			>
				...
				<textarea
					name='content'
					className={`w-full px-3 py-2 rounded-md resize-none overflow-y-scroll !text-black`}
					rows={5}
				...
				<div className='relative'>
					<textarea
						name='code'
						className={`w-full px-3 py-2 rounded-md resize-none overflow-y-scroll !text-black relative`}
						rows={5}
						placeholder='Код'
						onChange={handleChange}
						value={formData.code || ''}
				...
				<div className='flex flex-col gap-1'>
					...
					<div className='flex items-center gap-1'>
						...
						{comment.files && comment.files?.length < 9 && (
				...
				<div className='flex flex-col gap-1'>
					...
					<div className='flex items-center gap-1'>
						...
						{comment.attachments && comment.attachments?.length < 9 && (
							<label className='flex items-center gap-2 cursor-pointer'>
